...
범용 LLM(ChatGPT 등)이 이미지 생성을 잘 하는 만큼, 스케치 분할 역시 잘 할 수 있을 것이라고 생각하였고 InkLayer가 수행하는 장면 스케치 인스턴스 분할의 효용성에 의문이 생김.
...
- 동일한 장면에 대해 ChatGPT에 인스턴스 단위로 분리해 색을 입히라는 과제를 주고, InkLayer 출력과 나란히 비교함.
...
겉보기 품질에 속지 않고 stroke fidelity와 editability를 기준으로 보면, 범용 LLM은 재생성이라는 구조적 한계 때문에 이 과제에 부적합하고 전용 모델인 InkLayer가 더 낫다고 볼 수 있음.
